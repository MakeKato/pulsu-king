...
export(String, FILE, "*.tscn") var next_scene_path = "res://World0.tscn"
export(Vector2) var player_spawn_position = Vector2(550, 550)
...
func _on_body_entered(body):
	if body.is_in_group("Player"):  # Hero.tscn must be in the "Player" group
		...
		get_tree().change_scene(next_scene_path)  # Load World0.tscn
